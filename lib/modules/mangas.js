...
(function(root, factory){
    ...
    class Chapter {
                ...
                if (t.data.images.length < 1)
                {
                    ...
                    try {
                        fetch(t.url, {
                            ...
                            referrer: (new URL(t.url)).origin + '/',
                            referrerPolicy: "strict-origin-when-cross-origin"})
                                .then(resp => handleFetchError(resp))
                                .then(resp => resp.text())
        ...
        getPDF(progress, signal, timeout = 20, waitAfter = 0){
            ...
            return new Promise((resolve, reject) => {
                ...
                progress = progress || {current: 0, total: 1, label: ''};
                ...
                if (progress.aborted)
                {
                    return rej(new Error('Download aborted'));
                }
                ...
                this.getImages().then(images => {
                    ...
                    if (images.length < 1)
                    {
                        return rej(new Error('Cannot download ' + this.label));
                    }
                    ...
                    Promise.allSettled(images.map((url, index) => {
                        ...
                        this.createPDF(imgs, progress)
                                .then(pdf => {
                                    let cnt = Math.floor(waitAfter / 1000);
                                    if (cnt > 0)
                                    {
                                        progress.label = 'Please wait ' + cnt + 's';
