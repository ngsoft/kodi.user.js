            ...
            if(!this.data.pdf){
                ...
                for (let i = 0; i < images.length; i++) {
                    try {
                        ...
                        fetch(images[i], {referrer: this.url, headers: {'Content-Type': 'image/jpeg,image/png'}
                            }).then(r => r.arrayBuffer()).then(buffer => console.debug(isJPEG(buffer)));
