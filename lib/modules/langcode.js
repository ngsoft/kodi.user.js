...
(function (root, factory) {
        ...
        if (re instanceof RegExp) {
            ...
            for (i = 0; i < database.length; i++) {
                ...
                if (re.test(item.name))
                    return item;
                else if (re.test(item.alpha2))
                    return item;
                else if (re.test(item.alpha3))
                    return item;
                for (j = 0; j < item.alt.length; j++) {
                    if (re.test(item.alt[j]))
                        return item;
                }
    ...
    /**
     * Find a lang
     * @param {string|RegExp} lang
     * @returns {Promise} 
     */
    function find(lang){

        if (lang instanceof RegExp)
            return clone(findUsingRegex(lang) || und);

        if (!isString(lang)) {
            throw new Error('invalid argument lang: not a string|RegExp');
        }
        let result = und, len = lang.length, lower = lang.toLowerCase();
        if (len === 2)
            result = map2.get(lower) || result;
        else if (len === 3)
            result = map3.get(lower) || result;
        else if (RE_TAGS.test(lower))
            result = mapTags.get(lower) || result;
        else
            result = map.get(lower) || result;
