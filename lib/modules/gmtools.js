...
(function (root, factory) {
    ...
    async function getBlob(resourceName){

        let b64link;


        if (b64link = exports.getResource(resourceName)) {


            let
                    type = b64link.substring(b64link.indexOf(':') + 1, b64link.indexOf(';')),
                    b64 = b64link.substring(b64link.indexOf(',') + 1),
                    text = atob(b64);

            return new Blob([text], {type: type});
        }

        throw new Error('Invalid resource ' + resourceName);
    }

    async function getText(resourceName){
        return  await getBlob(resourceName).then(blob => blob.text());
    }

    async function getURL(resourceName, seconds = 0){

        let url = URL.createObjectURL(await getBlob(resourceName));
    ...
    function loadCSS(resourceName, id, root){
        ...
        return new Promise((resolve, reject) => {
            ...
            if (url = exports.getResource(resourceName)) {
                ...
                reject(new Error('Invalid resource ' + resourceName));
            }
    ...
    exports.resource = {getBlob, getText, loadCSS, getURL};
