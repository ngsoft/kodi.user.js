...
(function (root, factory) {
    ...
    let name = 'storage', deps = ['utils', 'gmtools'];
...
}(typeof self !== 'undefined' ? self : this, function (exports, utils, gmtools, undef) {


    const { uniqid, isPlainObject, JSON, isString, isFunction, document } = utils;
    ...
    class DataStore {
        ...
        onValueChange(name, listener) {
            if (isString(name) && isFunction(listener)) {
                ...
                this._elem.addEventListener('valuechange', handler);
            ...
            if (isString(name) && this._listeners[name]) {
                this._listeners[name] = this._listeners[name].filter(item => {
                    if (listener === item.listener || listener === undef) {
                        this._elem.removeEventListener('valuechange', item.handler);
                        return false;
                    }
                    return true;
                });
